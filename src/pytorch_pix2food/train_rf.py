...
from dataset.mydataset import RFDataset, OriginDataset, getTrainAndVal
...
if __name__ == '__main__':
    ...
    food_dataset = OriginDataset(trainConfig["training"]["data_root"],
    ...
    for epoch in range(trainConfig["training"]["num_epochs"]):
        ...
        for pixImg, trueImg, bboxImg in loader_train:
            if pixImg.shape[0] != 4:
                break
            # --- model update step --- #
    ...
    for pixImg, trueImg, bboxImg in loader_val:
        if pixImg.shape[0] != 4:
            break
        # --- model update step --- #
        pix2Food.feedInput(pixImg, trueImg)
